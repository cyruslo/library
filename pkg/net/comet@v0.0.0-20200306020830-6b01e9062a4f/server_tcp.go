...
import (
	"context"
	"git.huoys.com/middle-end/library/pkg/net/comet/internal/bucket"
	...
	xtime "git.huoys.com/middle-end/library/pkg/net/comet/internal/time"
	"git.huoys.com/middle-end/library/pkg/net/comet/proto"
	"git.huoys.com/middle-end/library/pkg/net/comet/internal/proxy"
...
func (s *Server) authTCP(conn *net.TCPConn, rr *bufio.Reader, p *proto.Payload) (err error) {
	for {
		...
		if p.Type == int32(proto.Request) && p.Seq == 0 {
			break
		} else {
			log.Error("tcp request seq(%d) not auth", p.Seq)
		}
	}
	token := string(p.Body[:])
